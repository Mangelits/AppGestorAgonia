...
def crear_html_informe(tipo, archivo_json):
        ...
        js_fila_tramo = """
            if (enCristo.length > 0 && enCruz.length > 0) {
                ...
                if (sinDescanso || sobrecarga) {
                    label = `⚠️ <strong style='color:#ff4757'>✝️ Cruz (${enCruz.join('+')}) [Doble Carga]</strong>`;
                } else {
    ...
    <body>
        ...
        <div class="container" id="informe-content">
            <div class="header">
                <img src="bandera_tercio_npj.png" alt="Escudo Mayordomía" class="logo-img" onerror="this.style.display='none'">
                <h1>OFS Muy Ilustre Mayordomía de Nuestro Padre Jesús Nazareno</h1>
                <h2>ORDEN PROCESIONAL - {tipo}</h2>
                <h3 style="color:#5c164e;">Tercio del Cristo de la Agonía y María Magdalena</h3>
                <p>Emitido: {fecha}</p>
            </div>
            ...
            <div class="leyenda">
                ...
                <div class="leyenda-item"><span class="caja bg-rojo"></span> Doble Carga (Cruz y Trono seguidos)</div>
            </div>
    """
